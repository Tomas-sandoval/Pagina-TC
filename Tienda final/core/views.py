...
def registrarme(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.save()
            premium='Aprovecha los mejores descuento especiales por ser cliente PREMIUM' 
            mensaje=f'Cuenta de usuario: "{usuario.username}"ha sido creada con exito'
...
def misdatos(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.usuario_id = usuario.id
            perfil.save()
            if perfil.tipo_usuario in ['Administador', 'Superusuario']:
                tipo_cuenta = perfil.tipo_usuario
            else:
                tipo_cuenta = 'CLIENTE PPREMIUM' if perfil.subscrito else 'cliente'
...
def boleta(request, nro_boleta):
    ...
    if Boleta.objects.filter(nro_boleta=nro_boleta).exists():
        ...
        if boleta_es_del_usuario or request.user.is_staff:
            ...
            messages.error(request, f'Lo siento la boleta Nro{nro_boleta} pertenece a {boleta.cliente}')
            boleta = None

    else:
        messages.error(request, f'La boleta nro {nro_boleta} no existe en la base de datos')
...
def ventas(request):
    ...
    historial = []
    for boleta in boletas:  # Cambiar 'boletas' a 'boleta' en el bucle
        boleta_historial = {
            'nro_boleta': boleta.nro_boleta,
            'nro_cliente': f'{boleta.cliente.usuario.first_name} {boleta.cliente.usuario.last_name}',
...
def usuarios(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.usuario_id = usuario.id
            perfil.save()
            messages.success(request, f'El usuario {usuario.first_name} {usuario.last_name} fue guardado correctamente')
...
def enviar_correo_cambio_password(request, user, password):
    try:
        ...
        subject = 'Cambio de contraseña Gummy Games Shop'
